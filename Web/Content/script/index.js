...
/********************* WxText文本 ************************/
//添加微信文本
function modifyWxText() {
    var text = $("#wxText").val();
    var type = "add";
    var id = $("#btnWxText").attr("tabindex");
    if (id > 0) {
        type = "update";
    }
    if ($.trim(text).length > 0) {
        $.ajax({
            type: "POST",
            url: "MyHandler.ashx",
            data: { method: "modifyWxText", id: id, text: text, type: type },
            success: function (result) {
                var result = $.parseJSON(result);
                if (result.success) {
                    show("编辑成功！");
    ...
    if (id) {
        $.ajax({
            ...
            success: function (result) {
                var result = $.parseJSON(result);
                if (result.success) {
                    $("#wxText").val(result.data.Content);
                } else {
                    show(result.error, false);
                }
            }
        });
        $("#btnWxText").html('<i class="glyphicon glyphicon-pencil"></i>修改');
        $("#btnWxText").attr("tabindex", id);
        $("#btnWxText").addClass("btn-warning");
        $("#btnWxText").removeClass("btn-success");
        $("#wxText").val("");
    } else {
        $("#btnWxText").html('<i class="glyphicon glyphicon-plus"></i>添加');
        $("#btnWxText").attr("tabindex", -1);
        $("#btnWxText").addClass("btn-success");
        $("#btnWxText").removeClass("btn-warning");
        $("#wxText").val("");
    }
}
